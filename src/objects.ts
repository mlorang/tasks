...
export function toMarkdown(question: Question): string {
    ...
    const len: number = question.options.length;
    const options = question.options.map((str: string): string =>
        str !== question.options[len - 1]
            ? "- " + str.toString() + "\n"
            : "- " + str.toString()
    );
...
export function renameQuestion(question: Question, newName: string): Question {
    const questionCopy = { ...question, name: newName };
    return questionCopy;
}
...
export function publishQuestion(question: Question): Question {
    const bool: boolean = question.published;
    const questionCopy = { ...question, published: !bool };
    return questionCopy;
}
